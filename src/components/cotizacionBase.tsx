...
export function NewQuote({ onViewChange }: NewQuoteProps) {
  ...
  const cotizacionData = {
    ...
    fecha: "2025-10-05",
    productos: [
      { nombre: "Impermeabilizante Acrílico", cantidad: 3, precio: 450 },
      { nombre: "Sellador Hidrofugante", cantidad: 2, precio: 300 },
    ],
    total: 1950,
  };
  ...
  const generatePDF = async () => {
    try {
      toast.info("Generando PDF de cotización...");

      // Enviar los datos de la cotización al backend
      ...
      // El backend devolverá la ruta del PDF generado
      ...
      link.download = pdfPath.split("/").pop(); // nombre del archivo
      link.click();
    } catch (error) {
      console.error(error);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-slate-200 rounded-lg">
            ...
            <CardContent>
              ...
              <div className="mb-4">
                <Label htmlFor="product-search" className="text-slate-700 pb-2">Buscar Producto</Label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-slate-400" />
                  <Input
                    id="product-search"
                      ...
                      {filteredProducts.map((product) => (
                        <div 
                          ...
                          onClick={() => {
                            ...
                            setProductSearch(product.nombre); // Muestra el nombre en el input
                          }}
              ...
              <div className="flex gap-4 mb-4">
                <div className="flex-1">
                  <Label htmlFor="product" className="text-slate-700 pb-2">Producto Seleccionado</Label>
                ...
                {selectedProduct && (
                  <div className="flex items-center gap-3 mt-3 p-3 border rounded-lg bg-slate-50">
                    <img
                      src={
                        selectedProduct.URLImagen
                          ? `http://localhost:5000/img/Productos/${selectedProduct.idProducto}.webp`
                          : "src/img/no_image.webp"
                      }
                      alt={selectedProduct.nombre}
                      className="w-12 h-12 object-cover rounded border"
                    />
                    <div className="flex-1">
                      <p className="font-medium text-slate-800">{selectedProduct.nombre}</p>
                      <p className="text-sm text-slate-600">
                        Stock: {selectedProduct.stock} • {selectedProduct.categoria}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="w-24">
                  <Label htmlFor="quantity" className="text-slate-700 pb-2">Cantidad</Label>
                  <Input
                    id="quantity"
                    type="number"
                    value={quantity}
